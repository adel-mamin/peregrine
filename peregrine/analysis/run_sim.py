...
import os,sys,argparse,subprocess,json,datetime,numpy
...
def runTracker(dopp, cp, lens):
    cmd = "python " + peregrinePath() + "/peregrine/analysis/tracking_loop.py -f 1bit_x2 -P 1 --profile low_rate "
    cmd = cmd + "-p " + cp + " -d " + dopp 
...
def processTrackResults(acc):
    data = np.genfromtxt(TRACK_DATA + ".PRN-1.l2c", dtype=float, delimiter=',', names=True)
    CN0  = data['CN0']
    ...
    dopErr = np.abs(dopp - i * 0.02 * acc * (signals.GPS.L2C.CENTER_FREQUENCY_HZ / signals.GPS.L1CA.CENTER_FREQUENCY_HZ))
...
def produce(lens, snr, dop, acc):
    ...
    avgCN0, lockRate, dopSigma1, dopSigma2, dopSigma3, maxDopErr = processTrackResults(acc)
...
def runCn0Range():
    ...
    for snr in snrRng:
        lockRate, dopSigma1, dopSigma2= produce(length, snr/10.0, doppler, acceleration)
...
def runDynamicLockRate():
    ...
    for snr in snrRng:
        ...
        while True:
            ...
            lockRate, dopSigma1, dopSigma2 = produce(length, snr/10.0, doppler, acc)
